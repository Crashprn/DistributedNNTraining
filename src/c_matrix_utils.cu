...
__global__
void m_index_to_one_hot_kernel(float* input, float* output, int rows, int cols)
{
    int row = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__
void m_softmax_kernel(float* input, int rows, int cols)
{
    int row = blockIdx.y * blockDim.y + threadIdx.y;
    if (row < rows)
    {
...
namespace cuda_matrix
{
    ...
    void m_add(float* d_input1, const float* d_input2, int rows, int cols)
    {
        ...
        dim3 blockDim(GRID_SIZE_X, GRID_SIZE_Y);
        int grid_x = static_cast<unsigned int>(ceil(1.0 * cols / blockDim.x));
        int grid_y = static_cast<unsigned int>(ceil(1.0 * rows / blockDim.y)); // One block for each row
        dim3 gridDim(grid_x, grid_y); // Calculate grid size based on rows and cols
    ...
    {
        // Define block and grid sizes
        dim3 blockDim(1, GRID_SIZE_X*GRID_SIZE_Y);
