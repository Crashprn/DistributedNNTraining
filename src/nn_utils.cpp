...
)
{
    ...
    m_copy(z1, a1, batch_size, h1); // z1 -> inter1
    m_Relu(a1, batch_size, h1); // (batch_size, h1) -> (batch_size, h1)

    // Layer 2
    m_mul(a1, w2, z2, batch_size, h1, h1, h2); // (batch_size, h1) * (h1, h2) -> (batch_size, h2)
    m_add_v(z2, b2, batch_size, h2, 1, h2); // (batch_size, h2) + (,h2) -> (batch_size, h2)
    m_copy(z2, a2, batch_size, h2); // z2 -> inter2 
    m_Relu(a2, batch_size, h2); // (batch_size, h2) -> (batch_size, h2)
    
    // Layer 3
    m_mul(a2, w3, z3, batch_size, h2, h2, h3); // (batch_size, h2) * (h2, h3) -> (batch_size, h3)
    m_add_v(z3, b3, batch_size, h3, 1, h3); // (batch_size, h3) + (,h3) -> (batch_size, h3)
    m_copy(z3, a3, batch_size, h3); // z3 -> inter3 
    m_Relu(a3, batch_size, h3); // (batch_size, h3) -> (batch_size, h3)

    // Layer 4
    m_mul(a3, w4, z4, batch_size, h3, h3, out_dim); // (batch_size, h3) * (h3, out_dim) -> (batch_size, out_dim)
...
)
{
    ...
    // Layer 4 Gradients
    // d4 = S(z4) - y
    m_sub(z4, y_targ, batch_size, out_dim); // (batch_size, out_dim) - (batch_size, out_dim) -> (batch_size, out_dim)
    ...
    m_mul(a3_T, z4, dw4, h3, batch_size, batch_size, out_dim); // (h3, batch_size) * (batch_size, out_dim) -> (h3, out_dim)
    ...
    m_mul(a2_T, delta3, dw3, h2, batch_size, batch_size, h3); // (h2, batch_size) * (batch_size, h3) -> (h2, h3)
    ...
    m_mul(a1_T, delta2, dw2, h1, batch_size, batch_size, h2); // (h1, batch_size) * (batch_size, h2) -> (h1, h2)
